...
- GGX: <img src="http://latex.codecogs.com/svg.latex?\frac{\alpha ^2}{\pi ((n \cdot h)^2(\alpha^2-1)+1)^2}">
  - <img src="http://lat ex.codecogs.com/svg.latex?\alpha"> is roughness.
...
$\frac{f\left(X\right) g\left(X\right) w_f\left(X\right)}{p_f\left(X\right)}+ \frac{f\left(Y\right) g\left(Y\right) w_g\left(Y\right)}{p_g\left(Y\right)}$

$X$ and $Y$ are the two samples we generated by sampling BRDF and direct light sampling in this iteration. $w_f$ and $w_g$ are special weighting functions chosen such that the expected value of this estimator.

A recomandation for weighting is power heuristic:

$w_f(X) = \frac{p^2_f(X)}{p^2_f(X) \cdot p^2_g(X)}$
